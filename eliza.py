...
Overview:
    This program will allow a user to have a conversation with the Eliza chatbot
    playing the role of psychotherapist. It will interact based on the users inputs
    and provide additional responses as necessary.

Sample conversation below:

    Hi, I'm Eliza, your psychotherapist. What is your name?
    My name is Asad.
    
    Hi Asad. You can type quit to end our conversation anytime. How can I help you today?
    I want to rule the world.
    
    Asad, why do you want to rule the world?
    I don't know, I think I crave power.
    
    Asad, why do you want to crave power?
    I want to destroy my enemies.
    
    Why are you interested in enemies?
    The enemies hurt me.
    
    There is a lot of research about The enemies. Are you aware of it?
...
Design:
    Eliza asks for user name and stores it for communication
    Each response that follows until the user enters
    exit or quit is used is used to parse the text entered
    and processed for a matching pattern.  A follow up response or
    question is framed based on the result of the match.  
    More details of the processing functionality can be found in
...
dictobj =  enchant.Dict("en_US")
prompt = "Hi, I'm Eliza, your psychotherapist. What is your name?\n"
...
while(flag == True):
    ...
    split_words = ConvertApostrophe(user_text).split(" ")
    #split_words = re.sub('^[a-z][A-Z]', '', user_text)
    ...
    for word in split_words:
        ...
        word = word.replace('-', '').replace('\'', '')
        word = word.replace('?', '').replace('(', '')
        if not (dictobj.check(word)):
            prompt = "I'm sorry, I did not recognize the word: " + word + ". Can you please try again?\n"
            reprompt = True;
            break
